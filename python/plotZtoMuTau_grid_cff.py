...
import TauAnalysis.Configuration.plotZtoMuTau_drawJobs_cfi as drawJobs

#--------------------------------------------------------------------------------
#
# Plot histograms for Z --> muon + tau-jet analysis
#
...
for sample in samples.FLATTENED_SAMPLES_TO_PLOT:
    ...
    if sample_info['type'].lower().find('mc') != -1:
        ...
        sample_pset.filterStatisticsLocation = cms.string('zMuTauAnalyzer/FilterStatistics/')
    else:
    ...
    if sample_info['factorize']:
        sample_pset.filterStatisticsLocation = cms.string(
            'zMuTauAnalyzer_factorizedWithMuonIsolation/FilterStatistics/')
...
for merge_name in samples.MERGE_SAMPLES.keys():
    ...
    new_pset = cms.PSet(
        dqmDirectories_input = cms.vstring([
            '/harvested/%s/zMuTauAnalyzer'%(sample) 
            for sample in merge_info['samples']
        ]),
        dqmDirectory_output = cms.string(
            '/harvested/%s/zMuTauAnalyzer'%(merge_name))
    )
    # Add the new PSet to our list
    _histAdderJobs["merge_%s" % (merge_name)] = new_pset

# Build the file loader
loadZtoMuTauSamples = cms.EDAnalyzer("DQMFileLoader", **_fileLoaderJobs)
# Build the histogram mergers
mergeSamplesZtoMuTau = cms.EDAnalyzer('DQMHistAdder', **_histAdderJobs)
...
loadAndFactorizeZtoMuTauSamples = cms.Sequence(loadZtoMuTauSamples)

loadZtoMuTau = cms.Sequence(loadAndFactorizeZtoMuTauSamples *
                             mergeSamplesZtoMuTau)

# Define plot processes and styles for each sample
# These get passed as kwargs to the DQMHistPlotter
_processesForZtoMuTauPlots = dict(
    (sampleName, cms.PSet(
...
drawJobTemplate = copy.deepcopy(drawJobs.plots_ZtoMuTau)
...
drawJobs.drawJobConfigurator_ZtoMuTau.setTemplate(drawJobTemplate)

plotZtoMuTau = cms.EDAnalyzer("DQMHistPlotter",
    processes = cms.PSet(**_processesForZtoMuTauPlots),
    ...
    drawJobs = drawJobs.drawJobConfigurator_ZtoMuTau.configure(),
    ...
    indOutputFileName = cms.string('plotZtoMuTau_#PLOT#.pdf')
)
